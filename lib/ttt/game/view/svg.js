(function($){
  ...
  var load_svg = function( callback ) {
    $.getScript("jquery.svg.js",function(){
      $.getScript("jquery.svgdom.js",function(){
        ...
        div.svg({ loadURL: "plain.svg",
                  onLoad: function(svg) {
                    ...
                    div.find(".ttt-cell").each(function(){
                      var v = $(this).get(0);
                      var classes = v.className && v.className.baseVal;
                      if(classes) {
        ...
        if(this.dom){
          ...
          $.each(state.board,function(k,v){
            if(!self.div.find(".ttt-cell."+k).hasClass("taken")){
              ...
              debug("s",scale_x,scale_y);


              var down_x = -(first_box.x + .5*first_box.width);
              var down_y = -(first_box.y + .5*first_box.height);
              
              var up_x = cell_box.x + .5*cell_box.width;
              var up_y = cell_box.y + .5*cell_box.height;
              ...
              debug("t",t);
              // marker.attr("transform","scale("+scale_x+","+scale_y+") translate("+offset_x+","+offset_y+")" );
              // marker.attr("transform","translate("+offset_x+","+offset_y+") scale("+scale_x+","+scale_y+")");
              // marker.attr("transform","translate("+offset_x+","+offset_y+")");
              // marker.attr("transform","scale("+scale_x+","+scale_y+")");
              // marker.attr("transform","matrix("+[ scale_x, 0,0, scale_y, offset_x * scale_x, offset_y * scale_y ].join(",") + ")" );
      ...
      watch: function watch(state) {
        if(!this._watcher) {
          ...
          this._watcher = function _watcher() {
            debug("click",$(this).attr("index"));
            // debug(template.xes.length);
            // debug(template.oes.length);
            self.game.move($(this).attr("index"));
          };
        }
        // debug(this.div.find(".ttt-cell"));
        // debug(this.div.find(".ttt-cell:not(.taken)"));
        if(this.player === state.current_player) {
          this.div.find(".ttt-cell:not(.taken)").click(this._watcher);
          // this.div.find(".ttt-cell")
        } else {
