                      ...
                      success: function(){
                        ...
                        div.svg({ loadURL: "plain.svg",
                                  onLoad: function(svg) {
                                    ...
                                    if(false){
                                      console.debug("HH0",$.print(div.find("rect")));
                                      console.debug("HH1",div.find("rect").get(0).xml);
                                      console.debug("HH1",div.find("rect").get(0).constructor+"");
                                      console.debug("HH1",div.find("rect").get(0).className+"");
                                      console.debug("HH2",div.find("rect").get(0).getAttribute("class"));
                                      console.debug("HH3",div.find("rect").get(0).className);
                                      console.debug("HHX",div.find("rect").eq(0).hasClass);
                                      console.debug("HH4",div.find("rect").eq(0).hasClass("ttt-cell"));
                                      console.debug("&&",$.print(div.find(".ttt-cell")));
                                    }
                                    div.find(".ttt-cell").each(function(){
                                      var v = $(this).get(0);
                                      //console.debug("v",$.print(v),v.className,v.className && v.className.baseVal,$(v).attr("class"),v.getAttribute("class"));
                                      var classes = v.getAttribute("class");
                                      if(classes) {
                                        classes = classes.split(/\s+/);
                                        //console.debug("cls",$.print(classes));
                                        for(var cls=0; cls<classes.length; cls++) {
                                          //console.debug("aa",$.print(cls));
                                          if(classes[cls].match(/\d\d/)){
                                            //console.debug("a",$.print(v));
      ...
      update: function update( state ) {
        ...
        if(this.dom){
          ...
          if(append) {
            ...
            //console.debug("cl",$.print(this.div)/*,this.div.html()*/,$.print(this.div.find(".ttt-cell")));
          }
        }
        this.watch(state);
      },
